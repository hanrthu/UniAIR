...
from models.encoders.RDE.common.layers import mask_zero, LayerNorm
...
class GABlock(nn.Module):
    ...
    def _pair_logits(self, z):
        ...
        return logits_pair
    # 这个spatial是假的spatial，它是直接投影过来的。这样做有什么意义呢？（可以用上局部坐标系？）
    ...
    def forward(self, R, t, x, z, mask):
        ...
        return x_updated, alpha


# Graph Attention Encoder
class GAEncoder(nn.Module):

    def __init__(self, node_feat_dim, pair_feat_dim, num_layers, num_heads=12, ga_block_opt={}):
        super(GAEncoder, self).__init__()
        self.blocks = nn.ModuleList([
            GABlock(node_feat_dim, pair_feat_dim, num_heads=num_heads, **ga_block_opt)
            for _ in range(num_layers)
        ])
        self.attns = []

    def forward(self, pos_atoms, res_feat, pair_feat, mask, need_attn=False):
        attns = []
        ...
        for block in self.blocks:
            res_feat, attn = block(R, t, res_feat, pair_feat, mask)
            attns.append(attn)
        if need_attn:
            return res_feat, attns
        else:
            return res_feat
