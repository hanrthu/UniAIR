...
from .flows.spline import ContextualCircularSplineFlow
from .flows.coupling import ContextualCircularSplineCouplingLayer
from .flows.container import ContextualSequentialFlow
from data.protein.residue_constants import chi_angles_atoms, lexico_to_full
...
class CircularSplineRotamerDensityEstimator(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.register_buffer('num_chis_of_aa', torch.tensor(
            data=[
                len(chi_angles_atoms[lexico_to_full[i]]) if i < len(chi_angles_atoms) else 0
                for i in range(21 + 1)
            ],
            dtype=torch.long,
        ))
