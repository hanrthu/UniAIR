...
class IntegrandNN(nn.Module):
    ...
    def forward(self, x, h):
        return self.net(torch.cat((x, h), 1)) + 1. # To make it strictly positive
...
class MonotonicNN(nn.Module):
    ...
    '''
    The forward procedure takes as input x which is the variable for which the integration must be made, h is just other conditioning variables.
    '''
    def forward(self, x, h):
